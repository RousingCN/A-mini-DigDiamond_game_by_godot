...
func _on_game_panel_game_over():
	TempData.buttonCountLabel.text=String.num(Data.row)+" X "+String.num(Data.col)
	TempData.diamondCountLabel.text=String.num(Data.diamondCount)
	TempData.costTimeLabel.text=Data.playTime
	TempData.invalidExcavationTimesLabel.text=String.num(Data.notDigDiamondTimes)+" 次"
	showOverMenu()
